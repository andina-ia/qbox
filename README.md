...
- **Admin** → login con correo `comercial@qboxmodular.com.ar` + clave. Edita textos,
  tarifas del cotizador, marcas, SEO y medios. Los cambios se guardan en **Supabase**
  (tabla `site_config`) vía la función `api/config.js` y el sitio los lee al cargar,
  así que impactan para todos los visitantes.

## Configuración (base de datos)
- La config editable del sitio vive en la tabla `site_config` de Supabase.
  Ejecutá `supabase/schema.sql` en el SQL editor de Supabase para crearla.
- Variables de entorno en Vercel (ver `.env.example`): `ADMIN_PASS`,
  `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` (y para imágenes: `GITHUB_TOKEN`,
  `GITHUB_OWNER`, `GITHUB_REPO`). La `service_role` key es server-side y no se expone.
